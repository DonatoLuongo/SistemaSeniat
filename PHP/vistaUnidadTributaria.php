...
<head>
    ...
    <title> Unidad Tributaria | SENIAT </title> <!-- Nombre de la Interfaz -->
...
<body>
    ...
    <main class="tabletarifas">
        ...
        <section class="table_body">
            <table>
                ...
                <tr>
                    ...
                    //Código para actualizar la Unidad Tributaria 
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                        try {
                            $unidad_tributaria = $_POST['unidad_tributaria'];

                            $stmt = $pdo->prepare("UPDATE unidad_tributaria SET valor = :valor");
                            $stmt->bindParam(':valor', $unidad_tributaria);
                            $stmt->execute();
                    ...
                    //Código para traer a la pantalla la Unidad Tributaria 
                    try {
                        $stmt = $pdo->query("SELECT valor FROM unidad_tributaria");
                        $resultados = $stmt->fetchAll(PDO::FETCH_ASSOC);

                        if (count($resultados) > 0) {
                            // echo '<h2>Unidad Tributaria</h2>';
                            // echo '<table border="1">';
                            ...
                            echo '<p>No hay valor de unidad tributaria registrado en la base de datos.</p>';
                        }
                    } catch (PDOException $e) {
        ...
        <!--Formulario de nueva Unidad Tributaria-->
        <form action="vistaUnidadTributaria.php" method="POST">
            <div class="user-details">
                <!-- Input para ingresar el valor nuevo -->
                <div class="input-box">
                    <label class="detail" for="nom">Actualice el valor de la Unidad Tributaria</label>
                    <input type="text" placeholder=" 9 " name="unidad_tributaria" id="cigarrillo" required>
                </div>
