...
<body>
    ...
    <main class="tabletarifas">
        ...
        <section class="table_body">
            <table>
                ...
                <tr>
                    ...
                    // Función para validar el nombre con la primera letra de cada palabra en mayúscula
                    function validarNombre($nombre)
                    {
                        return preg_match('/^([A-Z][a-z]*\s*)+$/', $nombre);
                    }

                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                        try {
                            $nombre_tarifa = $_POST['nombre_tarifa'];
                            $nombre_tarifa = ucwords(strtolower($nombre_tarifa));
                            $tarifa_licores_produccion = $_POST['tarifa_lico_produc'];

                            // Validar el nombre ingresado
                            if (!validarNombre($nombre_tarifa)) {
                                echo '<p class="alertas">El nombre ingresado no es válido. Por favor, asegúrese de que cada palabra comience con mayúscula.</p>';
                                return;
                            }

                            $stmt = $pdo->prepare("SELECT * FROM tarifa_licores_produccion WHERE nombre = :nombre");
                            $stmt->bindParam(':nombre', $nombre_tarifa);
                            $stmt->execute();
                            $tarifa_existente = $stmt->fetch();

                            // Verificar si el nombre existe en la base de datos
                            if (!$tarifa_existente) {
                                echo '<p class="alertas">El nombre ingresado no existe en la base de datos.</p>';
                                return;
                            }

                            $stmt = $pdo->prepare("UPDATE tarifa_licores_produccion SET tarifa = :tarifa WHERE nombre = :nombre");
                            $stmt->bindParam(':nombre', $nombre_tarifa);
                            $stmt->bindParam(':tarifa', $tarifa_licores_produccion);
                            $stmt->execute();

                            echo '<p class="alertas">Valor actualizado con éxito</p>';
        ...
        <form action="vistaTarifaLicoresProduc.php" method="POST">
            <div class="user-details">
                ...
                <div class="input-group">
                    ...
                    <div class="input-group-append">
                        ...
                        <input type="number" step="any" placeholder="0.0135" name="tarifa_lico_produc" id="cigarrillo" required>
                    </div>
