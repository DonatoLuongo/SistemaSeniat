...
// Función para generar el PDF con los datos de los productos de licores
function generarPDF($pdo)
{
    ...
    $pdf->AddPage();
     ...
     $pdf->Cell(0, 10, utf8_decode('Consulta de Licores'), 0, 1, 'C');
    ...
    try {
        ...
        if (count($resultados) > 0) {
            ...
            $pdf->Cell(70, 10, 'ID', 1, 0, 'C');
            $pdf->Cell(70, 10, 'Marca', 1, 1, 'C');
            $pdf->SetFont('Arial', '', 10);
            foreach ($resultados as $fila) {
                $pdf->Cell(70, 10, utf8_decode($fila['id']), 1, 0, 'C');
                $pdf->Cell(70, 10, utf8_decode($fila['marca']), 1, 1, 'C');
            }
        } else {
            $pdf->SetFont('Arial', 'B', 12);
            $pdf->Cell(0, 10, utf8_decode('No hay productos de licores registrados.'), 0, 1);
        }
